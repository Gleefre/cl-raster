...
(defun project-point-to-camera (camera point)
  (let* ((to-point-vector (vectors:v- (camera-get-center camera) point))
         (center-to-canvas-vector (vectors:v/ to-point-vector (vectors:v. to-point-vector (camera-get-direction camera))))
         (in-canvas-vector (vectors:v- center-to-canvas-vector (camera-get-direction camera)))
         (x-canvas-coordinate (vectors:v. (camera-get-x-vector camera) in-canvas-vector))
         (y-canvas-coordinate (vectors:v. (camera-get-y-vector camera) in-canvas-vector)))
    (vectors:vec2 (round (* (/ (1+ x-canvas-coordinate) 2) (camera-get-width camera)))
...
(defun render (scene camera)
    (let ((image (make-array (list (camera-width camera) (camera-height camera))
                             :initial-element '(0 0 0)))
          (depths (make-array (list (camera-width camera) (camera-height camera))
                              :initial-element NIL)))
      (loop for triangle in (scene-triangles scene)
            do (let* ((a-point (project-point-to-camera camera (first triangle)))
					  ...
					  (first b-point)
					  (first c-point))))
                      (maximal-x (min (camera-width camera)
				      (max
					(first a-point)
					(first b-point)
					(first c-point))))
