...
(defun triangle-contains (triangle point)
  (destructuring-bind (a b c)
    ...
    (let* ((AP (v:vunit* (v:v- point A)))
           (BP (v:vunit* (v:v- point B)))
           (CP (v:vunit* (v:v- point C))))
      (= (float-sign (cross-product-2d BP CP))
         (float-sign (cross-product-2d CP AP))
         (float-sign (cross-product-2d AP BP))))))
